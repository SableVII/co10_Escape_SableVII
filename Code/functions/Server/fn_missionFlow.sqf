...
if(isServer) then {
	...
	A3E_fnc_InlineEverybodyUnconscious = {
		private _return = 	(
							...
							OR 	(ACE_MedicalServer && (([] call A3E_fnc_GetPlayers) findIf {([_x] call ace_medical_status_fnc_hasStableVitals || _x getVariable ["ACE_isUnconscious", false] isEqualTo false);}) == -1)							
							);
		_return;
